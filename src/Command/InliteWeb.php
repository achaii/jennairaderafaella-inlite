...
class InliteWeb extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'jenna:inlite {name : The name of the module}';
     ...
     * @var string
     */
    protected $description = 'Create a new module type controller Modules directory. (e. jenna:inlite controller new_module)';
    ...
    protected function createModule(string $name)
    {
        ...
        try {
            ...
            if (!file_exists('resources/views/modules/' . ucfirst($words))) {
                mkdir('resources/views/modules/' . ucfirst($words), 0755, true);
            }

            // Create the module configuration file (_modules.yaml)
            touch(resource_path('views/modules/' . ucfirst($words) . '/' . Inlite::get_module_name_package_yaml()));

            $configFile = resource_path('views/modules/' . ucfirst($words) . '/' . Inlite::get_module_name_package_yaml());
            ...
            $source = File::directories(base_path('vendor/jennairaderafaella/the-moduless/src/dist/resources/views/modules/NameModule'));
            $destination = resource_path('views/modules/' . ucfirst($words));
